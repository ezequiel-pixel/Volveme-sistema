...
import { generarPdfBlob, armarLinkWhatsapp } from '../lib/generarPdf'
import { ArrowLeft, Download, Loader2, MapPin, Phone } from 'lucide-react'
    ...
    <div>
      ...
      <div className="ficha-operativa-contenedor bg-paper-card border border-rule rounded-lg">
        <div className="pres-page p-8" style={{ minHeight: '1000px' }}>
          ...
          {/* Dirección y contacto — lo primero que necesita el
              barista para llegar y saber a quién buscar */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 pb-6 border-b border-rule">
            <div>
              <p className="text-[10px] uppercase tracking-wide text-ink-light mb-0.5 flex items-center gap-1"><MapPin size={11} /> Dirección exacta</p>
              <p className="text-sm font-medium text-ink">{cotizacion?.lugar || evento.lugar || '—'}</p>
              {cotizacion?.lugar_lat && cotizacion?.lugar_lng && (
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${cotizacion.lugar_lat},${cotizacion.lugar_lng}`}
                  target="_blank" rel="noreferrer"
                  className="text-xs text-wine hover:underline print:hidden"
                >
                  Ver en Google Maps →
                </a>
              )}
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wide text-ink-light mb-0.5 flex items-center gap-1"><Phone size={11} /> Contacto en el lugar</p>
              {cotizacion?.contacto_nombre || cotizacion?.contacto_telefono ? (
                <>
                  <p className="text-sm font-medium text-ink">{cotizacion.contacto_nombre || '—'}</p>
                  {cotizacion.contacto_telefono && (
                    <p className="text-sm text-ink-mid">
                      {cotizacion.contacto_telefono}
                      {' · '}
                      <a
                        href={armarLinkWhatsapp(cotizacion.contacto_telefono, `¡Hola! Somos de Volveme, te escribo por el evento de hoy.`)}
                        target="_blank" rel="noreferrer" className="text-wine hover:underline print:hidden"
                      >
                        WhatsApp
                      </a>
                    </p>
                ...
                <p className="text-sm text-ink-light">No cargado</p>
              )}
            </div>
          </div>
