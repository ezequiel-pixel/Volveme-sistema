...
const defaultDia = () => ({ modo: 'horario', fecha: '', horaInicio: '08:00', horaFin: '18:00', duracionHoras: '', horaArmado: '' })
...
export default function NuevaCotizacion() {
    ...
    async function precargar() {
      ...
      if (diasCot && diasCot.length) {
        setDias(diasCot.map((d) => ({
          ...
          horaArmado: d.hora_armado?.slice(0, 5) || '',
        })))
      }
  ...
  async function handleSubmit(e) {
    ...
    const { data: nuevaCot, error: errCot } = await supabase.from('cotizaciones').insert({
      ...
      contacto_nombre: inputs.contacto_nombre || null,
      contacto_telefono: inputs.contacto_telefono || null,
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 items-start">
          <div className="space-y-6">
            <SectionCard icon={Users} title="Datos del evento" color="wine">
              ...
              <Row>
                <Field label="Contacto en el lugar (nombre)">
                  <input value={inputs.contacto_nombre} onChange={(e) => update('contacto_nombre', e.target.value)} className="input" placeholder="Quién recibe al barista" />
                </Field>
                <Field label="Contacto en el lugar (teléfono)">
                  <input value={inputs.contacto_telefono} onChange={(e) => update('contacto_telefono', e.target.value)} className="input" placeholder="11 5555-5555" />
                </Field>
              </Row>
            </SectionCard>
                  ...
                  <div key={i} className="bg-paper-card border border-rule rounded p-3 space-y-2">
                    ...
                    {(dia.modo || 'horario') === 'horario' ? (
                      <div className="flex items-end gap-2">
                        ...
                        <Field label="Armado (opcional)">
                          <input type="time" value={dia.horaArmado} onChange={(e) => updateDia(i, 'horaArmado', e.target.value)} className="input" title="A qué hora tiene que estar el barista armando, antes de que arranque el servicio" />
                        </Field>
                      </div>
                    ) : (
