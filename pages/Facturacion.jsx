  ...
  return (
    <>
      ...
      <div className="md:hidden space-y-2">
        {filas.map((f) => (
          <div key={f.evento.id} className="border border-rule rounded-lg p-4 bg-paper-card">
            <div className="flex items-start justify-between gap-3 mb-2">
              <p className="font-medium text-ink">{f.evento.clientes?.nombre || f.evento.nombre}</p>
              <EstadoPago pendiente={f.pendiente} />
            </div>
            <p className="text-xs text-ink-light mb-2">{new Date(f.evento.fecha + 'T00:00:00').toLocaleDateString('es-AR')}</p>
            <div className="grid grid-cols-3 gap-2 text-xs mb-3">
              <div><p className="text-ink-light">Corresponde</p><p className="text-ink-mid font-medium">{money(f.esperado)}</p></div>
              <div><p className="text-ink-light">Cobrado</p><p className="text-ink-mid font-medium">{money(f.cobrado)}</p></div>
              <div><p className="text-ink-light">Pendiente</p><p className="text-ink font-medium">{money(Math.max(0, f.pendiente))}</p></div>
            </div>
            {f.pendiente > 0 && (
              <button onClick={() => onRegistrar(f)} className="flex items-center gap-1.5 bg-wine text-paper text-xs rounded px-3 py-1.5">
                <Plus size={13} /> Registrar cobro
              </button>
            )}
          </div>
...
function TablaStaff({ filas, onRegistrar }) {
  ...
  return (
    <>
      ...
      <div className="md:hidden space-y-2">
        {filas.map((f, i) => (
          <div key={i} className="border border-rule rounded-lg p-4 bg-paper-card">
            <div className="flex items-start justify-between gap-3 mb-1">
              <p className="font-medium text-ink">{f.asignacion.staff?.nombre || '—'}</p>
              <span className="text-xs text-ink-light flex-shrink-0">{f.horas.toFixed(1)}hs</span>
            </div>
            <p className="text-xs text-ink-light mb-2">{f.asignacion.eventos?.clientes?.nombre || f.asignacion.eventos?.nombre || '—'}</p>
            {f.asignacion.staff?.alias_mercado_pago && (
              <p className="text-xs text-ink-light mb-2">MP: {f.asignacion.staff.alias_mercado_pago}</p>
            )}
            {f.asignacion.costo_extra > 0 && (
              <p className="text-xs text-orange mb-2">+{money(f.asignacion.costo_extra)} {f.asignacion.costo_extra_desc || 'extra'}</p>
            )}
            <div className="grid grid-cols-3 gap-2 text-xs mb-3">
              <div><p className="text-ink-light">Corresponde</p><p className="text-ink-mid font-medium">{money(f.esperado)}</p></div>
              <div><p className="text-ink-light">Pagado</p><p className="text-ink-mid font-medium">{money(f.pagado)}</p></div>
              <div><p className="text-ink-light">Pendiente</p><p className="text-ink font-medium">{money(Math.max(0, f.pendiente))}</p></div>
            </div>
            {f.pendiente > 0 && (
              <button onClick={() => onRegistrar(f)} className="flex items-center gap-1.5 bg-wine text-paper text-xs rounded px-3 py-1.5">
...
function TablaProveedores({ filas, onRegistrar }) {
  ...
  return (
    <>
      ...
      <div className="md:hidden space-y-2">
        {filas.map((f) => (
          <div key={f.compra.id} className="border border-rule rounded-lg p-4 bg-paper-card">
            <div className="flex items-start justify-between gap-3 mb-1">
              <p className="font-medium text-ink">{f.compra.insumos?.nombre || '—'}</p>
              {f.compra.proveedores?.telefono && (
                <a
                  href={armarLinkWhatsapp(f.compra.proveedores.telefono, `¡Hola! Te escribo por el pago de ${f.compra.insumos?.nombre || 'tu pedido'}.`)}
                  target="_blank" rel="noreferrer" className="text-wine flex-shrink-0"
                >
            ...
            <p className="text-xs text-ink-light mb-2">
              {f.compra.proveedores?.nombre_fantasia || '—'}
              {f.compra.proveedores?.alias_pago && ` · MP: ${f.compra.proveedores.alias_pago}`}
            </p>
            <div className="grid grid-cols-3 gap-2 text-xs mb-3">
              <div><p className="text-ink-light">Corresponde</p><p className="text-ink-mid font-medium">{money(f.esperado)}</p></div>
              <div><p className="text-ink-light">Pagado</p><p className="text-ink-mid font-medium">{money(f.pagado)}</p></div>
